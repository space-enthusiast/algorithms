...
NUM bino_coef_recur(NUM n, NUM k) {
    if (n == k || k == 0) {
        return 1;
    }

    return bino_coef_recur(n - 1, k) + bino_coef_recur(n - 1, k - 1);
}

NUM bino_coef_dp(NUM n, NUM k) {
...
NUM bino_coef_idea(NUM n, NUM k) {
...
int main() {
    ...
    PRT_NAME("bino_coef_recur(4, 2)", bino_coef_recur(4, 2))
    PRT_NAME("bino_coef_dp(4, 2)", bino_coef_dp(4, 2))
}